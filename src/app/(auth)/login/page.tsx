...
import { Sparkles, Loader2, KeyRound, Mail, LogIn, AlertCircle } from "lucide-react";
import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";

export default function LoginPage() {
  const { user, loginWithGoogle, loginWithPassword, loading } = useAuth();
        ...
        <div className="w-full max-w-md space-y-6">
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Akses dashboard perencanaan agenda tim NexaCode.
            </p>
          </div>
          ...
          <div className="border border-border rounded-2xl p-6 bg-slate-50/50 dark:bg-card/50 shadow-sm space-y-5">
            {loginMode === "google" ? (
              <button
                onClick={handleGoogleLogin}
                disabled={isLoggingIn}
                className="w-full flex items-center justify-center gap-3 bg-white hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-800 text-foreground border border-border py-2.5 px-4 rounded-xl font-bold shadow-xs transition-all cursor-pointer disabled:opacity-50"
              >
                {isLoggingIn ? (
              ...
              <form onSubmit={handlePasswordLogin} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-muted-foreground">Username atau Email</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Misal: nisa atau nisa@gmail.com"
                      className="w-full pl-10 pr-3 py-2 text-sm bg-white dark:bg-slate-900 border border-border rounded-xl focus:bg-white outline-none focus:border-primary-500 transition-all text-foreground font-semibold"
                      required
                    />
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  disabled={isLoggingIn}
                  className="w-full flex items-center justify-center gap-2 bg-primary-500 hover:bg-primary-600 text-white py-2.5 px-4 rounded-xl font-bold shadow-sm transition-all cursor-pointer disabled:opacity-50"
                >
                  {isLoggingIn ? (
            ...
            <div className="flex items-center gap-2 border-t border-border/60 pt-4 select-none">
              <input
                type="checkbox"
                id="rememberMe"
                checked={rememberMe}
                ...
                className="text-primary-500 focus:ring-primary-500 h-4 w-4 cursor-pointer rounded border-border bg-white"
              />
              <label htmlFor="rememberMe" className="text-xs font-bold text-muted-foreground cursor-pointer">
                Ingat Saya (Simpan login di browser)
              </label>
            </div>
            
            <div className="text-[11px] text-center text-muted-foreground leading-relaxed">
              Hanya anggota terdaftar (whitelist) yang dapat masuk ke dasbor NexaPlanner. Jika masuk lewat password pertama kali, gunakan username dan password default (prefix email Anda, misal email: nisa@gmail.com, password default: nisa).
            </div>
          </div>
        </div>
      </div>
